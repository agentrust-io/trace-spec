...
| `reproducibility` | OPTIONAL. Claim that a named deterministic function of the run, re-executed over a pinned input closure, yields a transcript with the stated digest. See §3.1.4. <!-- CHANGED: #249 - new optional reproducibility claim --> |: |
...
<!-- CHANGED: #249 - reproducibility claim: definition, verifier holdings, re-execution outcome, appraisal method -->

A `software-only` record (§3.1.1) is defined by what it lacks: nothing measured the execution, and the party asserting the evidence also wrote it. Some software-only producers can offer positive evidence of a different kind. Where the producer's coordination logic is a deterministic function, anyone holding its inputs can run it again and compare. This block is the claim that they can, stated precisely enough for a verifier to check it and precisely enough that a producer cannot make it loosely.
...
The claim has three parts, all explicit in the record:

| Field | Required | Meaning |
|---|---|---|
| `function` | yes | Name under which the implementation at `code_identity` exposes the deterministic function re-executed. The implementation's own documentation defines how a verifier invokes it. |
...
The transcript is the function's complete output as a JSON value: every decision the coordination logic took, in order. Its shape belongs to the producer and is described by the profile or annex that describes the function (§4.4). What this section fixes is that it is a JSON value canonicalised with RFC 8785, so that two verifiers digest the same bytes, and that the canonicalisation is the one §3.1.3 and §3.2.2 already require, so a verifier carries exactly one.

1. A claim whose closure omits anything that can change the transcript is **malformed**. That is the definition of the claim, not a quality bar on it: a closure that does not pin what it claims to pin is not a weak claim, it is not a claim.
2. A reproducibility claim MUST NOT affect `runtime.platform`. A record carrying the block and no `origin` block is `self` and carries whatever platform value it actually earned. Re-executability is not attestation and does not become it.
3. The record signature MUST cover `reproducibility`, under the canonicalisation in §3.2.2.

**Verifier holdings.** A verifier MUST hold three things before it reports any outcome other than `not-attempted`, and none of them may be supplied by the producer's environment:

- the implementation at `code_identity`, obtained as a public artifact;
- every blob in `input_closure`, each resolved through its own `resolver` and each matching its own `digest`;
- the claim itself, from the signed record.

If re-execution requires state that only the producer's environment can supply, it is not reproduction. The claim is then an assertion about that environment, and belongs elsewhere in the record or nowhere. The operational form of the same rule: a verifier MUST supply the function nothing but the closure. A function that reads beyond it is reading something the claim did not pin, and the outcome is `not-attempted`, with the read named as the reason, because the re-run that could be completed is not the re-run the claim describes. This is the rule a reader will be tempted to relax, and it is the one that makes the claim decidable rather than aspirational. It is also why model interactions are inputs in the closure rather than something a verifier re-invokes: a model call cannot be re-run to the same answer, so the function's boundary is drawn around it and the recorded interaction is pinned by digest.

**Outcome.** The re-execution result is one of three values, and a verifier MUST report the one that occurred:

| Outcome | Meaning |
|---|---|
| `reproduced` | The re-run completed and the digest of its transcript equals `transcript_digest`. |
| `diverged` | The re-run completed and the digests differ. The result MUST record the verifier's observed digest, because divergence localises nothing by itself: producer tampering, verifier drift and an under-pinned closure are indistinguishable until a third party can compare transcripts. |
| `not-attempted` | A closure blob could not be resolved, `code_identity` could not be obtained, or the re-run did not run to completion. The result MUST carry the reason. |
...
Two optional members of the result give a `diverged` outcome something to compare against. `resolved_closure_digest` is the digest over the RFC 8785 canonical form of the closure as the re-execution actually consumed it: every input the function read, content-addressed the same way as `input_closure`. It differs from the digest of the claimed closure exactly when the function read something the claim did not pin, which is the under-pinned case, decidable from a single result. `verifier_code_identity` is the digest of the verifier's own implementation. It discriminates nothing on its own: it is the verifier naming its own build, self-asserted, and it carries no weight singly. It earns its place as a correlation key across results, since two verifiers at different implementations disagreeing on a closure whose resolved digest matched is verifier drift, and that reading is unavailable without it.

| `resolved_closure_digest` | `verifier_code_identity` | Reading |
|---|---|---|
| differs from the claimed closure | any | under-pinned closure |
| equals the claimed closure | differs between the disagreeing results | verifier drift |
| equals the claimed closure | same across the disagreeing results | no reproducing verifier found a benign explanation |

The third row is the only one that accuses, and it accuses on absence. The record cannot distinguish a tampering producer from a cause nobody has thought of yet, so the row states what was not found rather than what was done.
...
| `appraisal.re_execution` | when `method` is `re-execution` | The result block: `outcome`, one of `reproduced`, `diverged`, `not-attempted`; `observed_digest`, required when `outcome` is `diverged`; `reason`, required when `outcome` is `not-attempted`; and the optional `resolved_closure_digest` and `verifier_code_identity` described above. |
...
"appraisal": {
  ...
  "re_execution": {
    ...
    "resolved_closure_digest": "sha256:b0a1c2d3..."
  }
}
```

**Out of scope.** This section names no journal format, no transcript shape, and no way of storing or resolving a closure; those belong to the profile or annex that describes the function (§4.4). `runtime.measurement` is unchanged, and its meaning under `software-only` is a separate question.
