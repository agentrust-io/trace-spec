...
Status as of September 2026. Spec **v0.2** is current ([`spec/trace-v0.2.md`](spec/trace-v0.2.md)); the `agentrust-trace` reference SDK is at **0.11.0** and the conformance suite (`agentrust-io/trace-tests`) at **0.5.1**.
...
- **MITRE ATLAS cross-walk**: [`docs/crosswalks/mitre-atlas.md`](docs/crosswalks/mitre-atlas.md).
...
- **MCP profile (normative)**: the [requirements proposal](spec/mcp-profile-v0.3-draft.md) separates TRACE attempt identity, correlation metadata and signed declaration snapshots. Wire format, implementation and adoption remain pending before upstream contribution to MCP spec governance.
